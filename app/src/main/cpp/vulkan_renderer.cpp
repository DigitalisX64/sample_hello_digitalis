...
// SPIR-V shaders compiled from shaders/triangle.vert and shaders/triangle.frag
// at build time by glslangValidator. See CMakeLists.txt.
...
bool vulkan_init(VulkanState* state, ANativeWindow* window) {
    ...
    if (!create_shader_module(state->device, k_triangle_vert_spv, sizeof(k_triangle_vert_spv),
                               &vert_module) ||
        !create_shader_module(state->device, k_triangle_frag_spv, sizeof(k_triangle_frag_spv),
                               &frag_module)) {

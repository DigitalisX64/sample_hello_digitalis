...
internal fun createCompatibleBitmap(inputBitmap: Bitmap) =
    // Bitmap.getConfig() is nullable on newer SDKs; the toolkit only handles
    // ARGB_8888/A_8 inputs (validated upstream), so a non-null config is
    // guaranteed here — default to ARGB_8888 if the platform ever returns null.
    Bitmap.createBitmap(
        inputBitmap.width,
        inputBitmap.height,
        inputBitmap.config ?: Bitmap.Config.ARGB_8888,

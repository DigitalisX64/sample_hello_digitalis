...
/// Simple result type replacing std::expected (unavailable in NDK 25 libc++).
struct BenchmarkResult {
  bool has_value_;
  std::chrono::nanoseconds value_;
  BenchmarkError error_;

  static BenchmarkResult success(std::chrono::nanoseconds ns) {
    return {true, ns, {}};
  }
  static BenchmarkResult failure(BenchmarkError err) {
    return {false, {}, err};
  }
  bool has_value() const { return has_value_; }
  std::chrono::nanoseconds value() const { return value_; }
  BenchmarkError error() const { return error_; }
};
 ...
 */
[[nodiscard]] BenchmarkResult BenchmarkMatrixMultiplication(Backend backend);
